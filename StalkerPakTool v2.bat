...
:: Check if file/folder was dragged onto script first
if not "%~1"=="" (
    set "REPAK_PATH=%~dp0repak.exe"
    set "input_path=%~f1"
    if /i "%~x1"==".pak" (
        echo Unpacking "%input_path%"...
        "!REPAK_PATH!" -a !AES_KEY! unpack "!input_path!" || (
...
echo.
echo To pack/unpack files, drag and drop a .pak file or folder onto this script.
echo.
...
if !CONFLICT_COUNT! equ 0 echo Happy hunting stalker.
if exist "!TEMP_DIR!" rmdir /s /q "!TEMP_DIR!" 2>nul
